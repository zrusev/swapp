...
const Starships = () => {
    ...
    const { data: stats, loading: loadingStats, error: errorStats } = useQuery(STATS_QUERY, {
    ...
    if (errorData || errorStats) return (<div style={{color: 'white', margin: '5em' }}>{errorData.message}</div>);
